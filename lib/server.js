...
function adapt(handler) {
  ...
  return async function adapter(req, res) {
    ...
    try {
      ...
      new Request({ method, path, query, headers, body })
      ...
      /* Basic anatomy of an error */
      let statusCode = 500,
          statusMessage = http.STATUS_CODES[500],
          details, stack

      /* Kinds of errors we handle */
      if (error instanceof HttpError) {
        ...
        details = (error || 'Unknow').toString()
      }

      /* Write the error response */
      let response = { statusCode, statusMessage, details, stack }
      let body = new Buffer(JSON.stringify(response), 'utf8')
      ...
      res.end(body, (error) => {
        if (error) log.error(`Error responding to ${req.method} ${req.url}`, error)
      })
    }
  }
}
